...
class DreamCartPoleSwingUpEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.world_model = SimpleWorldModel(
            obs_size=self.real_obs_size, hidden_size=self.hidden_size)
        ...
        self.world_model.load_model(
            "./log/learn_cartpole.pepg.16.384.best.json"
        )  # ignores agent stuff near the end
    ...
    def _step(self, action):
        ...
        x_dot_dot, theta_dot_dot = self.world_model.predict_dynamics(
            self.obs, action)
    ...
    def _reset(self):
        def init_state():
            x, x_dot, theta, theta_dot = np.random.normal(
    ...
    def _render(self, mode='human', close=False):
        ...
        self.pole_bob_trans.set_translation(-self.l * np.sin(theta),
                                            self.l * np.cos(theta))
