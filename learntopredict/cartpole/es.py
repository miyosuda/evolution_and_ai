...
class OpenES:
    ...
    def ask(self):
        ...
        # antithetic sampling
        if self.antithetic:
            self.epsilon_half = np.random.randn(self.half_popsize,
                                                self.num_params)
    ...
    def tell(self, reward_table_result):
        ...
        change_mu = 1. / (self.popsize * self.sigma) * np.dot(
            self.epsilon.T, normalized_reward)
...
class PEPG:
    ...
    def ask(self):
        ...
        self.epsilon = np.random.randn(self.batch_size,
                                       self.num_params) * self.sigma.reshape(
                                           1, self.num_params)
    ...
    def tell(self, reward_table_result):
        ...
        if (self.sigma_alpha > 0):
            ...
            reward_avg = (
                reward[:self.batch_size] + reward[self.batch_size:]) / 2.0
            rS = reward_avg - b
            delta_sigma = (np.dot(rS,
                                  S)) / (2 * self.batch_size * stdev_reward)
