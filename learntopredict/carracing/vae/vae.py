...
class ConvVAE(object):
    ...
    def _build_graph(self):
        ...
        with self.g.as_default():
            ...
            self.x = tf.placeholder(
                tf.float32, shape=[None, 64, 64, self.num_channel])
            ...
            self.logvar = tf.layers.dense(
                h, self.z_size, name="enc_fc_log_var")
            ...
            if self.is_training:
                ...
                self.kl_loss = -0.5 * tf.reduce_sum(
                    (1 + self.logvar - tf.square(self.mu) - tf.exp(
                        self.logvar)),
                    reduction_indices=1)
                self.kl_loss = tf.maximum(self.kl_loss,
                ...
                grads = self.optimizer.compute_gradients(
                    self.loss)  # can potentially clip gradients here.
    ...
    def encode_mu_logvar(self, x):
        (mu, logvar) = self.sess.run(
            [self.mu, self.logvar], feed_dict={self.x: x})
        return mu, logvar
    ...
    def get_random_model_params(self, stdev=0.5):
        ...
        for s in mshape:
            ...
            rparam.append(
                np.random.standard_cauchy(s) * stdev)  # spice things up
        return rparam
